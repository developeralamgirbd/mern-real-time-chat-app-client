...
export const verifyEmailRequest = async (email, otp)=>{
    try {
        ...
        const {data} =  await axios.get(`/auth/${email}/${otp}`);
        toast.success(data.message)
        return true
    }catch (e) {
        if (e.response.status === 400){
...
export const resetPasswordRequest = async (email, otp, password, confirmPassword)=>{
    try {
        ...
        const {data} =  await axios.patch(`/auth/${email}/${otp}`, {password, confirmPassword});
...
export const passwordChangeRequest = async (values)=>{
    try {
       ...
       const {data} = await axios.patch('/auth/password', values);
        if (data.result.modifiedCount > 0){
